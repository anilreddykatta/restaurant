	...
	function UserService ( $http ) {
		...
		function GetByUserNameAndPassword ( username, password ) {
			return $http.post ( '/api/users/login', {
				username: username,
				password: password
			} ).then ( handleSuccess, handleError ( 'Error in authenticating user' ) );
		}

		function GetByUserNameAndToken ( username, token ) {
			return $http.post ( '/api/users/login/social', {
				username: username,
				token: token
			} ).then ( handleSuccess, handleError ( "Error in authenticating user with google" ) );
		}
		...
		function UserChangePassword ( password, email, token ) {
			return $http.post ( '/api/users/login/changepassword', {
				email: email,
				token: token,
				password: password
			} ).then ( handleSuccess, handleError ( "Error in Signing out" ) );
		}
